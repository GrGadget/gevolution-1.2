    ...
    int &restartcount, set<long> *IDbacklog)
{
    ...
    if ((ext = strstr (ic.pclfile[0], ".h5")) != NULL)
    {
        ...
        get_fileDsc_local (filename + ".h5", numpcl, dummy1, dummy2,
                           fd.numProcPerFile);
    ...
    else
    {
        ...
        if (sim.baryon_flag == 1)
            pcls_cdm->parts_info ()->mass
                = cosmo.Omega_cdm / (Real)sim.numpcl[0];
        else
            pcls_cdm->parts_info ()->mass
    ...
    if (ic.restart_cycle >= 0)
    {
        ...
        if (parallel.isRoot ())
        {
            sprintf (line, "%s%s_background.dat", sim.output_path,
            ...
            else
            {
                ...
                else if (line[0] != '#')
                {
                    COUT << COLORTEXT_YELLOW << " /!\\ warning"
                         << COLORTEXT_RESET
                         << ": file for background output has "
                            "unexpected "
                            "format! "
                ...
                else
                {
                    fprintf (bgfile, "# background statistics\n# cycle "
                                     ...
                                     "        conformal H/H0  phi(k=0) "
                                     "      T00(k=0)\n");
                    fwrite ((const void *)buf.data (), sizeof (char),
                            buf.length (), bgfile);
                    fclose (bgfile);
                    buf.clear ();
                }
        ...
        for (i = 0; i < sim.num_lightcone; i++)
        {
            if (parallel.isRoot ())
            {
                if (sim.num_lightcone > 1)
                    sprintf (line, "%s%s%d_info.dat", sim.output_path,
                             sim.basename_lightcone, i);
                else
                    sprintf (line, "%s%s_info.dat", sim.output_path,
                ...
                else
                {
                    ...
                    if (sim.num_lightcone > 1)
                        sprintf (line, "%s%s%d_info.bin", sim.output_path,
                                 sim.basename_lightcone, i);
                    else
                        sprintf (line, "%s%s_info.bin", sim.output_path,
                                 sim.basename_lightcone);
                ...
                && d - tau + 0.5 * dtau_old > 0.)
            {
                for (p = 0; p < 1 + sim.baryon_flag + cosmo.num_ncdm; p++)
                {
                    ...
                    if (parallel.isRoot ())
                    {
                        ...
                        {
                            if (p == 0)
                                sprintf (line, "%s%s%d_%04d_cdm",
                                         sim.output_path,
                                         sim.basename_lightcone, i,
                                         ic.restart_cycle);
                            else if (p == 1 && sim.baryon_flag > 0)
                                sprintf (line, "%s%s%d_%04d_b", sim.output_path,
                                         sim.basename_lightcone, i,
                                         ic.restart_cycle);
                            else
                                sprintf (
                                    ...
                                    ic.restart_cycle, p - 1 - sim.baryon_flag);
                        }
                        else
                        {
                            if (p == 0)
                                sprintf (line, "%s%s_%04d_cdm", sim.output_path,
                                         sim.basename_lightcone,
                                         ic.restart_cycle);
                            else if (p == 1 && sim.baryon_flag > 0)
                                sprintf (line, "%s%s_%04d_b", sim.output_path,
                                         sim.basename_lightcone,
                                         ic.restart_cycle);
                            else
                                sprintf (
                                    line, "%s%s_%04d_ncdm%d", sim.output_path,
                                    sim.basename_lightcone, ic.restart_cycle,
                                    p - 1 - sim.baryon_flag);
                        }
                        ...
                        {
                            COUT << COLORTEXT_YELLOW << " /!\\ warning"
                                 << COLORTEXT_RESET << ": unable to open "
                                 << line
                                 << " for retrieving "
                                    "particle ID backlog"
                                 << endl;
                            count = 0;
                                 ...
                                 || fread (&hdr, sizeof (gadget2_header), 1,
                                           lcfile)
                                        != 1
                                 || fseek (lcfile,
                                           276l
                        ...
                        {
                            COUT << COLORTEXT_YELLOW << " /!\\ warning"
                                 << COLORTEXT_RESET << ": unable to read "
                                 << line
                                 << " for retrieving "
                                    "particle ID backlog"
                                 << endl;
                            ...
                            count = hdr.npartTotal[1]
                                    + ((long)hdr.npartTotalHW[1] << 32);
                    }
                    ...
                    while (count > 0)
                    {
                        if (count > PCLBUFFER)
                        {
                            if (parallel.isRoot ()
                                && fread (IDbuffer, 1,
                                          PCLBUFFER * GADGET_ID_BYTES, lcfile)
                                       != PCLBUFFER * GADGET_ID_BYTES)
                            {
                                COUT << COLORTEXT_RED << " /!\\ error"
                                     << COLORTEXT_RESET
                                     << ": unable "
                            ...
                            parallel.broadcast<char> (
                                (char *)IDbuffer, PCLBUFFER * GADGET_ID_BYTES,
                                0);
                            ...
                            for (int j = 0; j < PCLBUFFER; j++)
                            {
                                ...
                                if (IDlookup.erase (
                                        (long)*(((int64_t *)IDbuffer) + j)))
                                    IDbacklog[p].insert (
                                        (long)*(((int64_t *)IDbuffer) + j));
#else
                                if (IDlookup.erase (
                                        (long)*(((int32_t *)IDbuffer) + j)))
                                    IDbacklog[p].insert (
                                        (long)*(((int32_t *)IDbuffer) + j));
#endif
                            }
                        ...
                        {
                            if (parallel.isRoot ()
                                && fread (IDbuffer, 1, count * GADGET_ID_BYTES,
                                          lcfile)
                                       != count * GADGET_ID_BYTES)
                            {
                                COUT << COLORTEXT_RED << " /!\\ error"
                                     << COLORTEXT_RESET
                                     << ": unable "
                            ...
                            parallel.broadcast<char> (
                                (char *)IDbuffer, count * GADGET_ID_BYTES, 0);
    ...
    while (snapcount < sim.num_snapshot
    ...
    while (restartcount < sim.num_restart
