                             ...
                             int noutputs)
{
    ...
    gradxi[0] = (1. - ref_dist[1]) * (1. - ref_dist[2])
                * ((*fields[i]) (sites[i] + 0) - (*fields[i]) (sites[i]));
    gradxi[1] = (1. - ref_dist[0]) * (1. - ref_dist[2])
                * ((*fields[i]) (sites[i] + 1) - (*fields[i]) (sites[i]));
    gradxi[2] = (1. - ref_dist[0]) * (1. - ref_dist[1])
                * ((*fields[i]) (sites[i] + 2) - (*fields[i]) (sites[i]));
    gradxi[0]
        += ref_dist[1] * (1. - ref_dist[2])
           * ((*fields[i]) (sites[i] + 1 + 0) - (*fields[i]) (sites[i] + 1));
    gradxi[1]
        += ref_dist[0] * (1. - ref_dist[2])
           * ((*fields[i]) (sites[i] + 1 + 0) - (*fields[i]) (sites[i] + 0));
    gradxi[2]
        += ref_dist[0] * (1. - ref_dist[1])
           * ((*fields[i]) (sites[i] + 2 + 0) - (*fields[i]) (sites[i] + 0));
    gradxi[0]
        += (1. - ref_dist[1]) * ref_dist[2]
           * ((*fields[i]) (sites[i] + 2 + 0) - (*fields[i]) (sites[i] + 2));
    gradxi[1]
        += (1. - ref_dist[0]) * ref_dist[2]
           * ((*fields[i]) (sites[i] + 2 + 1) - (*fields[i]) (sites[i] + 2));
    gradxi[2]
        += (1. - ref_dist[0]) * ref_dist[1]
           * ((*fields[i]) (sites[i] + 2 + 1) - (*fields[i]) (sites[i] + 1));
    gradxi[0] += ref_dist[1] * ref_dist[2]
                 * ((*fields[i]) (sites[i] + 2 + 1 + 0)
                    - (*fields[i]) (sites[i] + 2 + 1));
    gradxi[1] += ref_dist[0] * ref_dist[2]
                 * ((*fields[i]) (sites[i] + 2 + 1 + 0)
                    - (*fields[i]) (sites[i] + 2 + 0));
    gradxi[2] += ref_dist[0] * ref_dist[1]
                 * ((*fields[i]) (sites[i] + 2 + 1 + 0)
                    - (*fields[i]) (sites[i] + 1 + 0));
    ...
    if (noutputs > 0)
        *outputs = coeff
                   * sqrt (gradxi[0] * gradxi[0] + gradxi[1] * gradxi[1]
                           + gradxi[2] * gradxi[2]);
...
//   ic         settings for IC generation (datastructure containing
//   parameters)
//
...
inline double Pk_primordial (const double k, const icsettings &ic)
{
    return ic.A_s
           * pow (k / ic.k_pivot,
                  ic.n_s - 1.); // note that k_pivot is in units of inverse Mpc!
}
    ...
    if (parallel.grid_rank ()[0] == 0) // read file
    {
        ...
        *ptr = '\0';
        ...
        while (!feof (tkfile) && !ferror (tkfile))
        {
            ...
            if (sscanf (line, format, dummy, dummy + 1, dummy + 2) == 3
                && !feof (tkfile) && !ferror (tkfile))
            {
                if (dummy[kcol] < 0.)
                {
                    std::cerr << " proc#" << parallel.rank ()
...
void generateCICKernel (Field<Real> &ker, const long numpcl, float *pcldata,
                        const int numtile)
{
    ...
    renorm /= (Real) (numpcl * (long)numtile * (long)numtile * (long)numtile)
              / (Real) (linesize * linesize * linesize);
    ...
    for (i = 0; i < numpcl; i++)
    {
        for (oct = 0; oct < 8; oct++)
        {
            if (oct == 0)
            {
                if (linesize * pcldata[3 * i] / numtile >= 1.
                    || linesize * pcldata[3 * i + 1] / numtile >= 1.
                    || linesize * pcldata[3 * i + 2] / numtile >= 1.)
                    continue;
            ...
            {
                if (linesize * (1. - pcldata[3 * i]) / numtile >= 1.
                    || linesize * pcldata[3 * i + 1] / numtile >= 1.
                    || linesize * pcldata[3 * i + 2] / numtile >= 1.)
                    continue;
            ...
            {
                if (linesize * (1. - pcldata[3 * i]) / numtile >= 1.
                    || linesize * (1. - pcldata[3 * i + 1]) / numtile >= 1.
                    || linesize * pcldata[3 * i + 2] / numtile >= 1.)
                    continue;
            ...
            else if (oct == 4)
            {
                if (linesize * pcldata[3 * i] / numtile >= 1.
                    || linesize * pcldata[3 * i + 1] / numtile >= 1.
                    || linesize * (1. - pcldata[3 * i + 2]) / numtile >= 1.)
                    continue;
            ...
            {
                ker (x) += ww * wy * (1. - wz) * q1;
                x.setCoord ((linesize + sx) % linesize, 0,
                            (linesize + sz) % linesize);
                ker (x) -= ww * wy * (1. - wz);
                if (wx > 0.9)
                {
                    x.setCoord ((linesize - sx) % linesize, 0,
            ...
            {
                ker (x) += ww * (1. - wy) * wz * q1;
                x.setCoord ((linesize + sx) % linesize,
                            (linesize + sy) % linesize, 0);
                ker (x) -= ww * (1. - wy) * wz;
                if (wx > 0.9)
                {
                    x.setCoord ((linesize - sx) % linesize,
            ...
            if (x.setCoord (0, (linesize + sy) % linesize,
                            (linesize + sz) % linesize))
            {
                ker (x) += ww * (1. - wy) * (1. - wz) * q1;
                x.setCoord ((linesize + sx) % linesize,
                            (linesize + sy) % linesize,
                            (linesize + sz) % linesize);
                ker (x) -= ww * (1. - wy) * (1. - wz);
                if (wx > 0.9)
                {
    ...
    Particles_gevolution &pcls)
{
    ...
    for (ztile = (pcls.lattice ().coordSkip ()[0] * numtile)
                 / pcls.lattice ().size (2);
         ztile
         <= ((pcls.lattice ().coordSkip ()[0] + pcls.lattice ().sizeLocal (2))
             * numtile)
                / pcls.lattice ().size (2);
         ztile++)
    {
        ...
        for (ytile = (pcls.lattice ().coordSkip ()[1] * numtile)
                     / pcls.lattice ().size (1);
             ytile <= ((pcls.lattice ().coordSkip ()[1]
                        + pcls.lattice ().sizeLocal (1))
                       * numtile)
                          / pcls.lattice ().size (1);
             ytile++)
        {
            ...
            for (xtile = 0; xtile < numtile; xtile++)
            {
                ...
                {
                    part.pos[0]
                        = ((Real)xtile + partdata[3 * i]) / (Real)numtile;
                    part.pos[1]
                        = ((Real)ytile + partdata[3 * i + 1]) / (Real)numtile;
                    part.pos[2]
                        = ((Real)ztile + partdata[3 * i + 2]) / (Real)numtile;

                    part.ID = i
                              + numpart
                                    * (xtile
                                       + (long)numtile
                                             * (ytile + (long)numtile * ztile));
...
double applyMomentumDistribution (
    Particles_gevolution *pcls,
    unsigned int seed, float T_m, Field<Real> *delta)
{
    ...
    const float ql[] = {
        0.0f,         0.0453329523f, 0.0851601009f, 0.115766097f, 0.142169202f,
        0.166069623f, 0.188283033f,  0.209275639f,  0.229344099f, 0.248691555f,
        0.267464827f, 0.285774552f,  0.303706948f,  0.321331123f, 0.338703809f,
        0.355872580f, 0.372878086f,  0.389755674f,  0.406536572f, 0.423248791f,
        0.439917819f, 0.456567164f,  0.473218795f,  0.489893502f, 0.506611198f,
        0.523391180f, 0.540252353f,  0.557213439f,  0.574293166f, 0.591510448f,
        0.608884565f, 0.626435339f,  0.644183319f,  0.662149971f, 0.680357893f,
        0.698831041f, 0.717594982f,  0.736677192f,  0.756107381f, 0.775917886f,
        0.796144127f, 0.816825143f,  0.838004248f,  0.859729814f, 0.882056241f,
        0.905045149f, 0.928766878f,  0.953302387f,  0.978745698f, 1.00520708f,
        1.03281729f,  1.06173322f,   1.09214584f,   1.12429121f,  1.15846661f,
        1.19505456f,  1.23456031f,   1.27767280f,   1.32536981f,  1.37911302f,
        1.44124650f,  1.51592808f,   1.61180199f,   1.75307820f,  29.0f
    };

    const float qr[]
        = { 29.0f,       11.8477879f, 10.3339062f, 9.58550750f, 9.08034038f,
            8.69584518f, 8.38367575f, 8.11975908f, 7.89035001f, 7.68685171f,
            7.50352431f, 7.33634187f, 7.18236952f, 7.03940031f, 6.90573157f,
            6.78002122f, 6.66119176f, 6.54836430f, 6.44081188f, 6.33792577f,
            6.23919052f, 6.14416529f, 6.05246957f, 5.96377208f, 5.87778212f,
            5.79424263f, 5.71292467f, 5.63362289f, 5.55615183f, 5.48034280f,
            5.40604138f, 5.33310512f, 5.26140176f, 5.19080749f, 5.12120558f,
            5.05248501f, 4.98453932f, 4.91726547f, 4.85056276f, 4.78433177f,
            4.71847331f, 4.65288728f, 4.58747144f, 4.52212011f, 4.45672261f,
            4.39116154f, 4.32531058f, 4.25903193f, 4.19217309f, 4.12456265f,
            4.05600493f, 3.98627278f, 3.91509779f, 3.84215661f, 3.76705129f,
            3.68928014f, 3.60819270f, 3.52291720f, 3.43223655f, 3.33436084f,
            3.22646839f, 3.10364876f, 2.95592669f, 2.75624893f, 0.0f };

    const float f[]
        = { 0.0f,           0.0010042516f, 0.0034718142f, 0.00631345904f,
            ...
            0.397475563f,   0.413789108f,  0.432131941f,  0.453799050f,
            0.482830296f };
        ...
        if (pcls->field () (xPart).size != 0)
        {
                 ...
                 it != (pcls->field ()) (xPart).parts.end (); ++it)
            {
                ...
                while (true)
                {
                    ...
                    q = ql[i]
                        + 64.0f * (qr[i] - ql[i])
                              * ((float)r / (float)sitmo::prng_engine::max ());
                ...
                r1 = acos (
                    2. * ((float)prng () / (float)sitmo::prng_engine::max ())
                    - 1.);
                r2 = 2 * M_PI
                     * ((float)prng () / (float)sitmo::prng_engine::max ());
    ...
    PlanFFT<Cplx> *plan_Sij, parameter * /* params */, int & /* numparam */)
{
    ...
    if (ic.pkfile[0] != '\0') // initial displacements & velocities are derived
    // from a single power spectrum
    {
    ...
    // functions
    {
        ...
        if (ic.tkfile[0] == '\0')
        {
            ...
            loadTransferFunctions (class_background, class_perturbs, tk_d1,
                                   tk_t1, "tot", sim.boxsize, sim.z_in,
                                   cosmo.h);
        }
        else
#endif
            loadTransferFunctions (ic.tkfile, tk_d1, tk_t1, "tot", sim.boxsize,
                                   cosmo.h);
        ...
        for (size_t i = 0; i < tk_d1->size; i++) // construct phi
            temp1[i]
                = (1.5
                  ...
                  * M_PI
                  * sqrt (
                        Pk_primordial (tk_d1->x[i] * cosmo.h / sim.boxsize, ic)
                        / tk_d1->x[i])
                  / tk_d1->x[i];
        ...
        if (ic.tkfile[0] == '\0')
        {
            if (sim.gr_flag == gravity_theory::Newtonian)
            {
                ...
                loadTransferFunctions (class_background, class_perturbs, tk_d1,
                                       tk_t1, NULL, sim.boxsize,
                                       (sim.z_in + 0.01) / 0.99, cosmo.h);
            ...
            loadTransferFunctions (class_background, class_perturbs, tk_d1,
                                   tk_t1, "cdm", sim.boxsize, sim.z_in,
                                   cosmo.h);
        }
        else
#endif
            loadTransferFunctions (ic.tkfile, tk_d1, tk_t1, "cdm", sim.boxsize,
                                   cosmo.h); // get transfer functions for CDM
...
#ifdef HAVE_CLASS
            if (ic.tkfile[0] == '\0')
                loadTransferFunctions (class_background, class_perturbs, tk_d2,
                                       tk_t2, "b", sim.boxsize, sim.z_in,
                                       cosmo.h);
            else
#endif
                loadTransferFunctions (
                    ic.tkfile, tk_d2, tk_t2, "b", sim.boxsize,
                    cosmo.h); // get transfer functions for baryons
        ...
        if (sim.baryon_flag
            == 3) // baryon treatment = hybrid; compute displacement
        // & velocity from weighted average (sub-species)
        {
                            ...
                            = -3. * pkspline->y[i] / pkspline->x[i]
                              ...
                              - ((8. * cosmo.Omega_cdm * tk_d1->y[i]
                                  + (7. * cosmo.Omega_b - cosmo.Omega_cdm)
                                        * tk_d2->y[i])
                                 / (cosmo.Omega_cdm + cosmo.Omega_b) / 7.)
                                    * M_PI
                                    * sqrt (Pk_primordial (tk_d1->x[i] * cosmo.h
                                                               / sim.boxsize,
                                                           ic)
            ...
            if (sim.gr_flag == gravity_theory::GR)
            {
                for (std::size_t i = 0; i < tk_d1->size; i++)
                    temp1[i]
                        = -3. * pkspline->y[i] / pkspline->x[i] / pkspline->x[i]
                          - tk_d2->y[i] * M_PI
                                * sqrt (Pk_primordial (tk_d2->x[i] * cosmo.h
                                                           / sim.boxsize,
                                                       ic)
                                        / tk_d2->x[i])
                                / tk_d2->x[i];
            ...
            if (sim.gr_flag == gravity_theory::GR || vnbspline == NULL)
            {
                for (std::size_t i = 0; i < tk_d1->size; i++)
                    temp2[i]
                        = -a * tk_t2->y[i] * M_PI
                          * sqrt (Pk_primordial (
                                      tk_d2->x[i] * cosmo.h / sim.boxsize, ic)
                                  / tk_d2->x[i])
                          / tk_d2->x[i];
            }
            else
        ...
        if ((sim.baryon_flag == 1 && !(ic.flags & ICFLAG_CORRECT_DISPLACEMENT))
            || sim.baryon_flag == 3)
        {
            generateDisplacementField (*scalarFT, 0., tk_d2,
                                       (unsigned int)ic.seed,
                                       ic.flags & ICFLAG_KSPHERE);
            gsl_spline_free (tk_d2);
            plan_phi->execute (FFT_BACKWARD);
            phi->updateHalo (); // phi now contains the baryonic
                                // displacement
            plan_source->execute (FFT_FORWARD);
        }

        generateDisplacementField (*scalarFT, 0., tk_d1, (unsigned int)ic.seed,
                                   ic.flags & ICFLAG_KSPHERE);
        gsl_spline_free (tk_d1);
    }
    ...
    if (sim.baryon_flag == 1)
        pcls_cdm_info.mass
            = cosmo.Omega_cdm
              / (Real) (sim.numpcl[0] * (long)ic.numtile[0]
                        * (long)ic.numtile[0] * (long)ic.numtile[0]);
    else
        pcls_cdm_info.mass
            = (cosmo.Omega_cdm + cosmo.Omega_b)
              / (Real) (sim.numpcl[0] * (long)ic.numtile[0]
    ...
    pcls_cdm->initialize (pcls_cdm_info, pcls_cdm_dataType, &(phi->lattice ()),
                          boxSize);

    initializeParticlePositions (sim.numpcl[0], pcldata, ic.numtile[0],
                                 *pcls_cdm);
    i = MAX;
    if (sim.baryon_flag == 3) // baryon treatment = hybrid; displace particles
        // using both displacement fields
        pcls_cdm->moveParticles (displace_pcls_ic_basic, 1., ic_fields, 2, NULL,
                                 &max_displacement, &i, 1);
    else
        pcls_cdm->moveParticles (displace_pcls_ic_basic, 1., &chi, 1, NULL,
                                 &max_displacement, &i,
                                 1); // displace CDM particles

    sim.numpcl[0]
        *= (long)ic.numtile[0] * (long)ic.numtile[0] * (long)ic.numtile[0];
        ...
        pcls_b_info.mass
            = cosmo.Omega_b
              / (Real) (sim.numpcl[1] * (long)ic.numtile[1]
                        * (long)ic.numtile[1] * (long)ic.numtile[1]);
        pcls_b_info.relativistic = false;

        pcls_b->initialize (pcls_b_info, pcls_b_dataType, &(phi->lattice ()),
                            boxSize);

        initializeParticlePositions (sim.numpcl[1], pcldata, ic.numtile[1],
                                     *pcls_b);
        i = MAX;
        pcls_b->moveParticles (displace_pcls_ic_basic, 1., &phi, 1, NULL,
                               &max_displacement, &i,
                               1); // displace baryon particles

        sim.numpcl[1]
            *= (long)ic.numtile[1] * (long)ic.numtile[1] * (long)ic.numtile[1];
    ...
    if (ic.pkfile[0] == '\0') // set velocities using transfer functions
    {
        ...
        {
            generateDisplacementField (*scalarFT, 0., tk_t2,
                                       (unsigned int)ic.seed,
                                       ic.flags & ICFLAG_KSPHERE, 0);
            plan_phi->execute (FFT_BACKWARD);
            phi->updateHalo (); // phi now contains the baryonic
                                // velocity potential
            gsl_spline_free (tk_t2);
        ...
        generateDisplacementField (*scalarFT, 0., tk_t1, (unsigned int)ic.seed,
                                   ic.flags & ICFLAG_KSPHERE, 0);
        plan_chi->execute (FFT_BACKWARD);
        chi->updateHalo (); // chi now contains the CDM velocity potential
        gsl_spline_free (tk_t1);

        if (sim.baryon_flag == 3) // baryon treatment = hybrid; set velocities
            // using both velocity potentials
            maxvel[0]
                = pcls_cdm->updateVel (initialize_q_ic_basic, 1., ic_fields, 2)
                  / a;
        else
            maxvel[0] = pcls_cdm->updateVel (initialize_q_ic_basic, 1., &chi, 1)
                        / a; // set CDM velocities

        if (sim.baryon_flag == 1)
            maxvel[1] = pcls_b->updateVel (initialize_q_ic_basic, 1., &phi, 1)
                        / a; // set baryon velocities
    }
    ...
    for (int p = 0; p < cosmo.num_ncdm; p++) // initialization of non-CDM species
    {
        ...
        loadHomogeneousTemplate (ic.pclfile[1 + sim.baryon_flag + p],
                                 sim.numpcl[1 + sim.baryon_flag + p], pcldata);
        ...
        if (ic.pkfile[0] == '\0')
        {
            ...
            if (ic.tkfile[0] == '\0')
                loadTransferFunctions (class_background, class_perturbs, tk_d1,
                                       tk_t1, ncdm_name, sim.boxsize, sim.z_in,
                                       cosmo.h);
            else
#endif
                loadTransferFunctions (ic.tkfile, tk_d1, tk_t1, ncdm_name,
                                       sim.boxsize, cosmo.h);
            ...
            plan_source->execute (FFT_FORWARD);
            generateDisplacementField (*scalarFT, 0., tk_d1,
                                       (unsigned int)ic.seed,
                                       ic.flags & ICFLAG_KSPHERE);
            plan_chi->execute (
                FFT_BACKWARD); // chi now contains the displacement
                               // for the non-CDM species
            chi->updateHalo ();
            gsl_spline_free (tk_d1);

            plan_source->execute (FFT_FORWARD);
            generateDisplacementField (*scalarFT, 0., tk_t1,
                                       (unsigned int)ic.seed,
                                       ic.flags & ICFLAG_KSPHERE, 0);
            plan_phi->execute (
                FFT_BACKWARD); // phi now contains the velocity
                               // potential for the non-CDM species
            phi->updateHalo ();
            gsl_spline_free (tk_t1);
        }

        strcpy (pcls_ncdm_info[p].type_name, "part_simple");
        pcls_ncdm_info[p].mass
            = cosmo.Omega_ncdm[p]
              / (Real) (sim.numpcl[1 + sim.baryon_flag + p]
                        * (long)ic.numtile[1 + sim.baryon_flag + p]
                        * (long)ic.numtile[1 + sim.baryon_flag + p]
                        * (long)ic.numtile[1 + sim.baryon_flag + p]);
        pcls_ncdm_info[p].relativistic = true;

        pcls_ncdm[p].initialize (pcls_ncdm_info[p], pcls_ncdm_dataType,
                                 &(phi->lattice ()), boxSize);

        initializeParticlePositions (
            sim.numpcl[1 + sim.baryon_flag + p], pcldata,
            ic.numtile[1 + sim.baryon_flag + p], pcls_ncdm[p]);
        i = MAX;
        pcls_ncdm[p].moveParticles (displace_pcls_ic_basic, 1., &chi, 1, NULL,
                                    &max_displacement, &i,
                                    1); // displace non-CDM particles

        sim.numpcl[1 + sim.baryon_flag + p]
            *= (long)ic.numtile[1 + sim.baryon_flag + p]
               * (long)ic.numtile[1 + sim.baryon_flag + p]
               * (long)ic.numtile[1 + sim.baryon_flag + p];
        ...
        if (ic.pkfile[0]
            == '\0') // set non-CDM velocities using transfer functions
    ...
    {
        plan_source->execute (FFT_FORWARD);
        generateDisplacementField (*scalarFT, 0., pkspline,
                                   (unsigned int)ic.seed,
                                   ic.flags & ICFLAG_KSPHERE, 0);
#ifdef HAVE_CLASS
        if (ic.tkfile[0] == '\0')
            freeCLASSstructures (class_background, class_thermo,
                                 class_perturbs);
#endif
    }
    else
    ...
    // velocities using linear approximation
    {
        ...
        if (sim.baryon_flag)
            maxvel[1]
                = pcls_b->updateVel (initialize_q_ic_basic, rescale, &phi, 1)
                  / a;
    }
    ...
    for (int p = 0; p < cosmo.num_ncdm; p++)
    {
        ...
        if (ic.pkfile[0] != '\0') // if power spectrum is used instead of
                                  // transfer functions,
