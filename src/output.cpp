    ...
    )
{
    ...
    long * IDcombuf2;
    Site xsim;
    // int done_B = 0; // we don't do FFT here
#ifdef HAVE_HEALPIX
    int64_t pix, pix2, q;
    std::vector<int> pixbatch_id;
    std::vector<int> sender_proc;
    std::vector<int> pixbatch_size[3];
    std::vector<int> pixbatch_delim[3];
    int pixbatch_type;
    int commdir[2];
    ...
    for (i = 0; i < sim.num_lightcone; i++)
    {
        if (parallel.isRoot())
        {
            ...
            if (outfile == NULL)
            {
                std::cout << " error opening file for lightcone info!" << std::endl;
            }
            else if (cycle == 0)
            {
                if (sim.num_lightcone > 1)
        ...
        if (sim.out_lightcone[i] & MASK_GADGET && sim.lightcone[i].distance[0] > d - tau + 0.5 * dtau_old && sim.lightcone[i].distance[1] <= d - tau + 0.5 * dtau_old && d - tau + 0.5 * dtau_old > 0.)
        {
            ...
            if (sim.baryon_flag)
                hdr.mass[1] = (double) sim.tracer_factor[0] * cosmo.C_RHO_CRIT * cosmo.Omega_cdm * sim.boxsize * sim.boxsize * sim.boxsize / sim.numpcl[0] / GADGET_MASS_CONVERSION;
            else
                hdr.mass[1] = (double) sim.tracer_factor[0] * cosmo.C_RHO_CRIT * (cosmo.Omega_cdm + cosmo.Omega_b) * sim.boxsize * sim.boxsize * sim.boxsize / sim.numpcl[0] / GADGET_MASS_CONVERSION;
            ...
            if (sim.tracer_factor[0] > 0)
                pcls_cdm->legacy_saveGadget2(
                    h5filename + filename + "_cdm", 
                    hdr, 
                    sim.lightcone[i], 
                    d - tau, 
                    dtau, 
            ...
            {
                hdr.mass[1] = (double) sim.tracer_factor[1] * cosmo.C_RHO_CRIT * cosmo.Omega_b * sim.boxsize * sim.boxsize * sim.boxsize / sim.numpcl[1] / GADGET_MASS_CONVERSION;
                pcls_b->legacy_saveGadget2(
                    h5filename + filename + "_b", 
                    hdr, 
                    sim.lightcone[i], 
                    d - tau, 
                    dtau, 
            ...
            for (p = 0; p < cosmo.num_ncdm; p++)
            {
                ...
                cosmo.C_RHO_CRIT * cosmo.Omega_ncdm[p] * sim.boxsize * sim.boxsize * sim.boxsize / sim.numpcl[p+1+sim.baryon_flag] / GADGET_MASS_CONVERSION;
                pcls_ncdm[p].legacy_saveGadget2(
                    h5filename + filename + buffer, 
                    hdr, 
                    sim.lightcone[i], 
                    d - tau, 
                    dtau, 
                    ...
                    vertex, 
                    n, 
                    IDbacklog[p+1+sim.baryon_flag], 
                    IDprelog[p+1+sim.baryon_flag], 
                    phi, 
...
void Particles_gevolution::legacy_saveGadget2(
    std::string filename, 
    gadget2_header & hdr, 
    ...
    const int tracer_factor)
{
    ...
    typename std::list<particle>::iterator it;
