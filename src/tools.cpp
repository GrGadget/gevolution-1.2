...
void extractCrossSpectrum (Field<Cplx> &fld1FT, Field<Cplx> &fld2FT, Real *kbin,
                           ...
                           const bool deconvolve, const int ktype,
                           const int comp1, const int comp2)
{
    int i, weight;
    ...
    if (ktype == KTYPE_GRID)
    {
        ...
        {
            typek2[i]
                = 2. * (Real)linesize * sin (M_PI * (Real)i / (Real)linesize);
            typek2[i] *= typek2[i];
        }
    }
    else
    ...
    for (k.first (); k.test (); k.next ())
    {
        ...
        if (comp1 >= 0 && comp2 >= 0 && comp1 < fld1FT.components ()
            && comp2 < fld2FT.components ())
        {
        ...
        if (i < numbins)
        {
            ...
            pscatter[i]
                += weight * p.real () * p.real () * k2 * k2 * k2 / s / s;
            occupation[i] += weight;
        }
    }
    ...
    {
#ifdef SINGLE
        MPI_Reduce (MPI_IN_PLACE, (void *)kbin, numbins, MPI_FLOAT, MPI_SUM, 0,
                    parallel.lat_world_comm ());
        MPI_Reduce (MPI_IN_PLACE, (void *)kscatter, numbins, MPI_FLOAT, MPI_SUM,
                    0, parallel.lat_world_comm ());
        MPI_Reduce (MPI_IN_PLACE, (void *)power, numbins, MPI_FLOAT, MPI_SUM, 0,
                    parallel.lat_world_comm ());
        MPI_Reduce (MPI_IN_PLACE, (void *)pscatter, numbins, MPI_FLOAT, MPI_SUM,
                    0, parallel.lat_world_comm ());
#else
        MPI_Reduce (MPI_IN_PLACE, (void *)kbin, numbins, MPI_DOUBLE, MPI_SUM, 0,
                    parallel.lat_world_comm ());
        MPI_Reduce (MPI_IN_PLACE, (void *)kscatter, numbins, MPI_DOUBLE,
                    MPI_SUM, 0, parallel.lat_world_comm ());
        MPI_Reduce (MPI_IN_PLACE, (void *)power, numbins, MPI_DOUBLE, MPI_SUM,
                    0, parallel.lat_world_comm ());
        MPI_Reduce (MPI_IN_PLACE, (void *)pscatter, numbins, MPI_DOUBLE,
                    MPI_SUM, 0, parallel.lat_world_comm ());
#endif
        MPI_Reduce (MPI_IN_PLACE, (void *)occupation, numbins, MPI_INT, MPI_SUM,
                    0, parallel.lat_world_comm ());
        ...
        for (i = 0; i < numbins; i++)
        {
            ...
            {
                kscatter[i]
                    = sqrt (kscatter[i] * occupation[i] - kbin[i] * kbin[i])
                      / occupation[i];
                if (!std::isfinite (kscatter[i]))
                    kscatter[i] = 0.;
                kbin[i] = kbin[i] / occupation[i];
                power[i] /= occupation[i];
                pscatter[i]
                    = sqrt (pscatter[i] / occupation[i] - power[i] * power[i]);
                if (!std::isfinite (pscatter[i]))
                    pscatter[i] = 0.;
            }
        }
    }
    ...
    {
#ifdef SINGLE
        MPI_Reduce ((void *)kbin, NULL, numbins, MPI_FLOAT, MPI_SUM, 0,
                    parallel.lat_world_comm ());
        MPI_Reduce ((void *)kscatter, NULL, numbins, MPI_FLOAT, MPI_SUM, 0,
                    parallel.lat_world_comm ());
        MPI_Reduce ((void *)power, NULL, numbins, MPI_FLOAT, MPI_SUM, 0,
                    parallel.lat_world_comm ());
        MPI_Reduce ((void *)pscatter, NULL, numbins, MPI_FLOAT, MPI_SUM, 0,
                    parallel.lat_world_comm ());
#else
        MPI_Reduce ((void *)kbin, NULL, numbins, MPI_DOUBLE, MPI_SUM, 0,
                    parallel.lat_world_comm ());
        MPI_Reduce ((void *)kscatter, NULL, numbins, MPI_DOUBLE, MPI_SUM, 0,
                    parallel.lat_world_comm ());
        MPI_Reduce ((void *)power, NULL, numbins, MPI_DOUBLE, MPI_SUM, 0,
                    parallel.lat_world_comm ());
        MPI_Reduce ((void *)pscatter, NULL, numbins, MPI_DOUBLE, MPI_SUM, 0,
                    parallel.lat_world_comm ());
#endif
        MPI_Reduce ((void *)occupation, NULL, numbins, MPI_INT, MPI_SUM, 0,
                    parallel.lat_world_comm ());
    }
}
...
void extractPowerSpectrum (Field<Cplx> &fldFT, Real *kbin, Real *power,
                           ...
                           const int numbins, const bool deconvolve,
                           const int ktype)
{
    extractCrossSpectrum (fldFT, fldFT, kbin, power, kscatter, pscatter,
                          occupation, numbins, deconvolve, ktype);
}
#endif
...
void writePowerSpectrum (Real *kbin, Real *power, Real *kscatter,
                         ...
                         double a, const double z_target)
{
    if (parallel.isRoot ())
    {
        ...
        if (1. / a < z_target + 1.)
        {
            ...
            if (infile != NULL)
            {
                ...
                for (int i = 0; i < numbins; i++)
                    power2[i] = (1. - weight) * power2[i]
                                + weight * power[i] / rescalep;
            ...
            for (int i = 0; i < numbins; i++)
            {
                if (occupation[i] > 0)
                    ...
                    fprintf (outfile, "  %e   %e   %e   %e   %d\n",
                             kbin[i] / rescalek, power2[i],
                             kscatter[i] / rescalek,
                             pscatter[i] / rescalep / sqrt (occupation[i]),
                             occupation[i]);
#else
                    fprintf (outfile, "  %e   %e   %e   %e   %d\n",
                             kbin[i] / rescalek, power[i] / rescalep,
                             kscatter[i] / rescalek,
                             pscatter[i] / rescalep / sqrt (occupation[i]),
                             occupation[i]);
#endif
...
void computeVectorDiagnostics (Field<Real> &Bi, Real &mdivB, Real &mcurlB)
{
    ...
    {
        b1 = fabs ((Bi (x, 0) - Bi (x - 0, 0)) + (Bi (x, 1) - Bi (x - 1, 1))
                   + (Bi (x, 2) - Bi (x - 2, 2)))
             * linesize;
        if (b1 > mdivB)
            mdivB = b1;
        b1 = 0.5
             * (Bi (x, 0) + Bi (x + 0, 1) - Bi (x + 1, 0) - Bi (x, 1)
                + Bi (x + 2, 0) + Bi (x + 0 + 2, 1) - Bi (x + 1 + 2, 0)
                - Bi (x + 2, 1))
             * linesize;
        b2 = 0.5
             * (Bi (x, 0) + Bi (x + 0, 2) - Bi (x + 2, 0) - Bi (x, 2)
                + Bi (x + 1, 0) + Bi (x + 0 + 1, 2) - Bi (x + 2 + 1, 0)
                - Bi (x + 1, 2))
             * linesize;
        b3 = 0.5
             * (Bi (x, 2) + Bi (x + 2, 1) - Bi (x + 1, 2) - Bi (x, 1)
                + Bi (x + 0, 2) + Bi (x + 2 + 0, 1) - Bi (x + 1 + 0, 2)
                - Bi (x + 0, 1))
             * linesize;
                               ...
                               Real &mnormh)
{
    ...
    for (x.first (); x.test (); x.next ())
    {
        d1 = (hij (x + 0, 0, 0) - hij (x, 0, 0) + hij (x, 0, 1)
              - hij (x - 1, 0, 1) + hij (x, 0, 2) - hij (x - 2, 0, 2))
             * linesize;
        d2 = (hij (x + 1, 1, 1) - hij (x, 1, 1) + hij (x, 0, 1)
              - hij (x - 0, 0, 1) + hij (x, 1, 2) - hij (x - 2, 1, 2))
             * linesize;
        d3 = (hij (x + 2, 2, 2) - hij (x, 2, 2) + hij (x, 0, 2)
              - hij (x - 0, 0, 2) + hij (x, 1, 2) - hij (x - 1, 1, 2))
             * linesize;
        ...
        if (d1 > mtraceh)
            mtraceh = d1;
        d1 = sqrt (
            hij (x, 0, 0) * hij (x, 0, 0) + 2. * hij (x, 0, 1) * hij (x, 0, 1)
            + 2. * hij (x, 0, 2) * hij (x, 0, 2) + hij (x, 1, 1) * hij (x, 1, 1)
            + 2. * hij (x, 1, 2) * hij (x, 1, 2)
            + hij (x, 2, 2) * hij (x, 2, 2));
        if (d1 > mnormh)
            mnormh = d1;
    }
            ...
            for (w = -range; w <= range; w++)
            {
                ...
                if (vertex[n][0] < domain[0])
                {
                    if (vertex[n][1] < domain[1])
                    {
                        ...
                        {
                            if (sqrt ((vertex[n][0] - corner[0][0])
                                          * (vertex[n][0] - corner[0][0])
                                      + (vertex[n][1] - corner[0][1])
                                            * (vertex[n][1] - corner[0][1])
                                      + (vertex[n][2] - corner[0][2])
                                            * (vertex[n][2] - corner[0][2]))
                                > outer)
                                continue;
                        }
                        ...
                        {
                            if (sqrt ((vertex[n][0] - corner[4][0])
                                          * (vertex[n][0] - corner[4][0])
                                      + (vertex[n][1] - corner[4][1])
                                            * (vertex[n][1] - corner[4][1])
                                      + (vertex[n][2] - corner[4][2])
                                            * (vertex[n][2] - corner[4][2]))
                                > outer)
                                continue;
                        }
                        else if (sqrt ((vertex[n][0] - domain[0])
                                           * (vertex[n][0] - domain[0])
                                       + (vertex[n][1] - domain[1])
                                             * (vertex[n][1] - domain[1]))
                                 > outer)
                    ...
                    else if (vertex[n][1] > domain[4])
                    {
                        if (vertex[n][2] < domain[2])
                        {
                            if (sqrt ((vertex[n][0] - corner[2][0])
                                          * (vertex[n][0] - corner[2][0])
                                      + (vertex[n][1] - corner[2][1])
                                            * (vertex[n][1] - corner[2][1])
                                      + (vertex[n][2] - corner[2][2])
                                            * (vertex[n][2] - corner[2][2]))
                                > outer)
                                continue;
                        }
                        ...
                        {
                            if (sqrt ((vertex[n][0] - corner[6][0])
                                          * (vertex[n][0] - corner[6][0])
                                      + (vertex[n][1] - corner[6][1])
                                            * (vertex[n][1] - corner[6][1])
                                      + (vertex[n][2] - corner[6][2])
                                            * (vertex[n][2] - corner[6][2]))
                                > outer)
                                continue;
                        }
                ...
                else if (vertex[n][0] > domain[3])
                {
                    if (vertex[n][1] < domain[1])
                    {
                        if (vertex[n][2] < domain[2])
                        {
                            if (sqrt ((vertex[n][0] - corner[1][0])
                                          * (vertex[n][0] - corner[1][0])
                                      + (vertex[n][1] - corner[1][1])
                                            * (vertex[n][1] - corner[1][1])
                                      + (vertex[n][2] - corner[1][2])
                                            * (vertex[n][2] - corner[1][2]))
                                > outer)
                                continue;
                        }
                        ...
                        {
                            if (sqrt ((vertex[n][0] - corner[5][0])
                                          * (vertex[n][0] - corner[5][0])
                                      + (vertex[n][1] - corner[5][1])
                                            * (vertex[n][1] - corner[5][1])
                                      + (vertex[n][2] - corner[5][2])
                                            * (vertex[n][2] - corner[5][2]))
                                > outer)
                                continue;
                        }
                        else if (sqrt ((vertex[n][0] - domain[3])
                                           * (vertex[n][0] - domain[3])
                                       + (vertex[n][1] - domain[1])
                                             * (vertex[n][1] - domain[1]))
                                 > outer)
                    ...
                    else if (vertex[n][1] > domain[4])
                    {
                        if (vertex[n][2] < domain[2])
                        {
                            if (sqrt ((vertex[n][0] - corner[3][0])
                                          * (vertex[n][0] - corner[3][0])
                                      + (vertex[n][1] - corner[3][1])
                                            * (vertex[n][1] - corner[3][1])
                                      + (vertex[n][2] - corner[3][2])
                                            * (vertex[n][2] - corner[3][2]))
                                > outer)
                                continue;
                        }
                        ...
                        {
                            if (sqrt ((vertex[n][0] - corner[7][0])
                                          * (vertex[n][0] - corner[7][0])
                                      + (vertex[n][1] - corner[7][1])
                                            * (vertex[n][1] - corner[7][1])
                                      + (vertex[n][2] - corner[7][2])
                                            * (vertex[n][2] - corner[7][2]))
                                > outer)
                                continue;
                        }
                ...
                else
                {
                    ...
                    else if (vertex[n][2] - domain[5] > outer
                             || domain[2] - vertex[n][2] > outer)
                        continue;
                }

                if (sqrt ((corner[0][0] - vertex[n][0])
                              * (corner[0][0] - vertex[n][0])
                          + (corner[0][1] - vertex[n][1])
                                * (corner[0][1] - vertex[n][1])
                          + (corner[0][2] - vertex[n][2])
                                * (corner[0][2] - vertex[n][2]))
                        < inner
                    && sqrt ((corner[1][0] - vertex[n][0])
                                 * (corner[1][0] - vertex[n][0])
                             + (corner[1][1] - vertex[n][1])
                                   * (corner[1][1] - vertex[n][1])
                             + (corner[1][2] - vertex[n][2])
                                   * (corner[1][2] - vertex[n][2]))
                           < inner
                    && sqrt ((corner[2][0] - vertex[n][0])
                                 * (corner[2][0] - vertex[n][0])
                             + (corner[2][1] - vertex[n][1])
                                   * (corner[2][1] - vertex[n][1])
                             + (corner[2][2] - vertex[n][2])
                                   * (corner[2][2] - vertex[n][2]))
                           < inner
                    && sqrt ((corner[3][0] - vertex[n][0])
                                 * (corner[3][0] - vertex[n][0])
                             + (corner[3][1] - vertex[n][1])
                                   * (corner[3][1] - vertex[n][1])
                             + (corner[3][2] - vertex[n][2])
                                   * (corner[3][2] - vertex[n][2]))
                           < inner
                    && sqrt ((corner[4][0] - vertex[n][0])
                                 * (corner[4][0] - vertex[n][0])
                             + (corner[4][1] - vertex[n][1])
                                   * (corner[4][1] - vertex[n][1])
                             + (corner[4][2] - vertex[n][2])
                                   * (corner[4][2] - vertex[n][2]))
                           < inner
                    && sqrt ((corner[5][0] - vertex[n][0])
                                 * (corner[5][0] - vertex[n][0])
                             + (corner[5][1] - vertex[n][1])
                                   * (corner[5][1] - vertex[n][1])
                             + (corner[5][2] - vertex[n][2])
                                   * (corner[5][2] - vertex[n][2]))
                           < inner
                    && sqrt ((corner[6][0] - vertex[n][0])
                                 * (corner[6][0] - vertex[n][0])
                             + (corner[6][1] - vertex[n][1])
                                   * (corner[6][1] - vertex[n][1])
                             + (corner[6][2] - vertex[n][2])
                                   * (corner[6][2] - vertex[n][2]))
                           < inner
                    && sqrt ((corner[7][0] - vertex[n][0])
                                 * (corner[7][0] - vertex[n][0])
                             + (corner[7][1] - vertex[n][1])
                                   * (corner[7][1] - vertex[n][1])
                             + (corner[7][2] - vertex[n][2])
                                   * (corner[7][2] - vertex[n][2]))
                           < inner)
                    continue; // domain lies within inner sphere

                rdom = 0.5
                       * sqrt ((domain[3] - domain[0]) * (domain[3] - domain[0])
                               + (domain[4] - domain[1])
                                     * (domain[4] - domain[1])
                               + (domain[5] - domain[2])
                                     * (domain[5] - domain[2]));
                dist = sqrt (
                    (0.5 * domain[0] + 0.5 * domain[3] - vertex[n][0])
                        * (0.5 * domain[0] + 0.5 * domain[3] - vertex[n][0])
                    + (0.5 * domain[1] + 0.5 * domain[4] - vertex[n][1])
                          * (0.5 * domain[1] + 0.5 * domain[4] - vertex[n][1])
                    + (0.5 * domain[2] + 0.5 * domain[5] - vertex[n][2])
                          * (0.5 * domain[2] + 0.5 * domain[5] - vertex[n][2]));

                if (dist <= rdom) // vertex lies within domain
                                  // enclosing sphere
                {
                    n++;
                    continue;
                ...
                if (((0.5 * domain[0] + 0.5 * domain[3] - vertex[n][0])
                     ...
                     + (0.5 * domain[2] + 0.5 * domain[5] - vertex[n][2])
                        ...
                        / dist
                    >= lightcone.opening) // center of domain lies
                                          // within opening
                {
                    n++;
                    continue;
                    ...
                    && acos (
                           ((0.5 * domain[0] + 0.5 * domain[3] - vertex[n][0])
                            ...
                            + (0.5 * domain[2] + 0.5 * domain[5] - vertex[n][2])
                                  * lightcone.direction[2])
                           / dist)
                               - acos (lightcone.opening)
                           <= acos ((outer * outer + dist * dist - rdom * rdom)
                                    / (2. * outer * dist))) // enclosing sphere
                                                            // within opening
                {
                    n++;
                    continue;
                ...
                if (dist <= outer
                    && acos (
                           ...
                           / dist)
                               - acos (lightcone.opening)
                           <= asin (rdom / dist)) // enclosing sphere
                                                  // within opening
                {
                    n++;
                }
