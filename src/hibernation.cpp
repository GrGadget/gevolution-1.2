...
void writeRestartSettings (metadata &sim, icsettings &ic, cosmology &cosmo,
                           ...
                           const int cycle, const int restartcount)
{
    ...
    if (restartcount >= 0)
        sprintf (buffer, "%s%s%03d.ini", sim.restart_path, sim.basename_restart,
                 restartcount);
    else
    ...
    {
        fprintf (outfile,
                 "# automatically generated settings for restart after "
                 "hibernation ");
        if (restartcount < 0)
        ...
        if (restartcount >= 0)
            ...
            buffer[0] = '\0';
        fprintf (outfile, "particle file      = %s%s%s_cdm.h5",
                 sim.restart_path, sim.basename_restart, buffer);
        if (sim.baryon_flag)
            fprintf (outfile, ", %s%s%s_b.h5", sim.restart_path,
                     sim.basename_restart, buffer);
        for (i = 0; i < cosmo.num_ncdm; i++)
        {
            if (sim.numpcl[1 + sim.baryon_flag + i] < 1)
                fprintf (outfile, ", /dev/null");
            else
                fprintf (outfile, ", %s%s%s_ncdm%d.h5", sim.restart_path,
        ...
        if (sim.gr_flag == gravity_theory::GR)
        {
            fprintf (outfile, "metric file        = %s%s%s_phi.h5",
                     sim.restart_path, sim.basename_restart, buffer);
            fprintf (outfile, ", %s%s%s_chi.h5", sim.restart_path,
                     sim.basename_restart, buffer);
        ...
        if (sim.out_snapshot)
        {
            ...
            if (sim.out_snapshot & MASK_PCLS)
            {
                ...
                if ((sim.out_snapshot & MASK_MULTI == 0
                     && sim.out_snapshot > MASK_DELTA)
                    || sim.out_snapshot - MASK_MULTI > MASK_DELTA)
                    fprintf (outfile, ", ");
            }
            if (sim.out_snapshot & MASK_DELTA)
            {
                ...
                if ((sim.out_snapshot & MASK_MULTI == 0
                     && sim.out_snapshot > MASK_DBARE)
                    || sim.out_snapshot - MASK_MULTI > MASK_DBARE)
                    fprintf (outfile, ", ");
            }
        ...
        if (sim.num_lightcone == 1)
        {
            fprintf (outfile, "lightcone vertex    = %lg, %lg, %lg\n",
                     sim.lightcone[0].vertex[0], sim.lightcone[0].vertex[1],
                     sim.lightcone[0].vertex[2]);
            ...
            fprintf (outfile, "lightcone distance  = %lg, %lg\n",
                     sim.lightcone[0].distance[1],
                     sim.lightcone[0].distance[0]);
            ...
            for (i = 0; i < sim.num_lightcone; i++)
            {
                ...
                if (sim.lightcone[0].z != 0)
                    fprintf (outfile, "lightcone %d redshift  = %lg\n", i,
                             sim.lightcone[0].z);
                fprintf (outfile,
                         "lightcone %d direction = %.15le, %.15le, "
                         "%.15le\n",
                         i, sim.lightcone[0].direction[0],
                         sim.lightcone[0].direction[1],
                         sim.lightcone[0].direction[2]);
