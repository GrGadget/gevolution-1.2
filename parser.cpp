...
static int sort_descending (const void *z1, const void *z2)
{
    return (*(double *)z1 > *(double *)z2)
               ? -1
               : ((*(double *)z1 < *(double *)z2) ? 1 : 0);
}
...
void saveParameterFile (const char *filename, parameter *params,
                        const int numparam, bool used_only)
{
#ifdef LATFIELD2_HPP
    if (parallel.isRoot ())
#endif
        ...
        if (strcmp (params[i].name, pname) == 0)
        {
            ...
            while ((comma = strchr (start, ',')) != NULL)
            {
                ...
                if (strcmp (item, "Phi") == 0 || strcmp (item, "phi") == 0)
                    ...
                    pvalue |= MASK_CHI;
                else if (strcmp (item, "Pot") == 0 || strcmp (item, "pot") == 0
                         || strcmp (item, "Psi_N") == 0
                         || strcmp (item, "psi_N") == 0
                         || strcmp (item, "PsiN") == 0
                         || strcmp (item, "psiN") == 0)
                    ...
                    pvalue |= MASK_TIJ;
                else if (strcmp (item, "rho_N") == 0
                         || strcmp (item, "rhoN") == 0)
                    pvalue |= MASK_RBARE;
                else if (strcmp (item, "hij") == 0 || strcmp (item, "GW") == 0)
                    pvalue |= MASK_HIJ;
                else if (strcmp (item, "Gadget") == 0
                         || strcmp (item, "Gadget2") == 0
                         || strcmp (item, "gadget") == 0
                         || strcmp (item, "gadget2") == 0)
                    pvalue |= MASK_GADGET;
                else if (strcmp (item, "multi-Gadget") == 0
                         || strcmp (item, "multi-Gadget2") == 0
                         || strcmp (item, "multi-gadget") == 0
                         || strcmp (item, "multi-gadget2") == 0)
                    pvalue |= MASK_GADGET | MASK_MULTI;
                else if (strcmp (item, "Particles") == 0
                         || strcmp (item, "particles") == 0
                         || strcmp (item, "pcls") == 0
                         || strcmp (item, "part") == 0)
                    pvalue |= MASK_PCLS;
                else if (strcmp (item, "cross") == 0
                         || strcmp (item, "X-spectra") == 0
                         || strcmp (item, "x-spectra") == 0)
                    pvalue |= MASK_XSPEC;
                else if (strcmp (item, "delta") == 0 || strcmp (item, "Ds") == 0
                         || strcmp (item, "D_s") == 0)
                    pvalue |= MASK_DELTA;
                else if (strcmp (item, "delta_N") == 0
                         || strcmp (item, "deltaN") == 0)
                    pvalue |= MASK_DBARE;
                else if (strcmp (item, "v") == 0
                         || strcmp (item, "velocity") == 0)
                    pvalue |= MASK_VEL;
            ...
            if (strcmp (start, "Phi") == 0 || strcmp (start, "phi") == 0)
                ...
                pvalue |= MASK_CHI;
            else if (strcmp (start, "Pot") == 0 || strcmp (start, "pot") == 0
                     || strcmp (start, "Psi_N") == 0
                     || strcmp (start, "psi_N") == 0
                     || strcmp (start, "PsiN") == 0
                     || strcmp (start, "psiN") == 0)
                pvalue |= MASK_POT;
                ...
                pvalue |= MASK_TIJ;
            else if (strcmp (start, "rho_N") == 0
                     || strcmp (start, "rhoN") == 0)
                pvalue |= MASK_RBARE;
            else if (strcmp (start, "hij") == 0 || strcmp (start, "GW") == 0)
                pvalue |= MASK_HIJ;
            else if (strcmp (start, "Gadget") == 0
                     || strcmp (start, "Gadget2") == 0
                     || strcmp (start, "gadget") == 0
                     || strcmp (start, "gadget2") == 0)
                pvalue |= MASK_GADGET;
            else if (strcmp (start, "multi-Gadget") == 0
                     || strcmp (start, "multi-Gadget2") == 0
                     || strcmp (start, "multi-gadget") == 0
                     || strcmp (start, "multi-gadget2") == 0)
                pvalue |= MASK_GADGET | MASK_MULTI;
            else if (strcmp (start, "Particles") == 0
                     || strcmp (start, "particles") == 0
                     || strcmp (start, "pcls") == 0
                     || strcmp (start, "part") == 0)
                pvalue |= MASK_PCLS;
            else if (strcmp (start, "cross") == 0
                     || strcmp (start, "X-spectra") == 0
                     || strcmp (start, "x-spectra") == 0)
                pvalue |= MASK_XSPEC;
            else if (strcmp (start, "delta") == 0 || strcmp (start, "Ds") == 0
                     || strcmp (start, "D_s") == 0)
                pvalue |= MASK_DELTA;
            else if (strcmp (start, "delta_N") == 0
                     || strcmp (start, "deltaN") == 0)
                pvalue |= MASK_DBARE;
            else if (strcmp (start, "v") == 0
                     || strcmp (start, "velocity") == 0)
                pvalue |= MASK_VEL;
                   ...
                   cosmology &cosmo, icsettings &ic)
{
    ...
    if ((!parseParameter (params, numparam, "mPk file", ic.pkfile)
         && !parseParameter (params, numparam, "Tk file", ic.tkfile)
         && ic.generator != ICGEN_READ_FROM_DISK
         && ic.generator != ICGEN_FALCONIC)
#else
    if ((!parseParameter (params, numparam, "mPk file", ic.pkfile)
    ...
    if (!parseParameter (params, numparam, "tiling factor", ic.numtile, i)
        && ic.generator != ICGEN_READ_FROM_DISK)
    {
        COUT << COLORTEXT_YELLOW << " /!\\ warning" << COLORTEXT_RESET
    ...
    for (i = 1; i < MAX_PCL_SPECIES; i++)
    {
        if (ic.numtile[i] < 0 && ic.generator != ICGEN_READ_FROM_DISK)
        {
            ...
            ic.numtile[i] = 1;
                 ...
                 && strcmp (ic.pclfile[i], "/dev/null") != 0)
            ic.numtile[i] = 1;
    }
    ...
    if (sim.baryon_flag == 1 && ic.numtile[1] <= 0
        && ic.generator != ICGEN_READ_FROM_DISK)
    {
        COUT << COLORTEXT_YELLOW << " /!\\ warning" << COLORTEXT_RESET
    ...
    {
        if (par_string[0] == 'i' || par_string[0] == 'I' || par_string[0] == 'b'
            || par_string[0] == 'B')
        {
    ...
    if (parseParameter (params, numparam, "fluid treatment", par_string))
    {
        ...
        else if (par_string[0] == 'c' || par_string[0] == 'C')
        {
            ...
            {
                COUT << COLORTEXT_YELLOW << " /!\\ warning" << COLORTEXT_RESET
                     << ": using fluid treatment = CLASS and "
                        "providing initial "
    ...
    if (ic.generator == ICGEN_READ_FROM_DISK)
    {
        ...
        if (parseParameter (params, numparam, "gevolution version",
                            ic.restart_version))
        {
    ...
    if (!parseParameter (params, numparam, "generic file base",
                         sim.basename_generic))
        sim.basename_generic[0] = '\0';

    if (!parseParameter (params, numparam, "snapshot file base",
    ...
    if (!parseParameter (params, numparam, "lightcone file base",
    ...
    if (!parseParameter (params, numparam, "hibernation path",
                         sim.restart_path))
        strcpy (sim.restart_path, sim.output_path);

    if (!parseParameter (params, numparam, "hibernation file base",
                         sim.basename_restart))
        strcpy (sim.basename_restart, "restart");

    parseParameter (params, numparam, "boxsize", sim.boxsize);
    if (sim.boxsize <= 0. || !std::isfinite (sim.boxsize))
    {
        COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET
    ...
    if (sim.numpts < 2 || !std::isfinite (sim.numpts))
    {
        COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET
    ...
    if (parseParameter (params, numparam, "downgrade factor",
                        sim.downgrade_factor))
    {
        if (sim.downgrade_factor < 1 || sim.downgrade_factor >= sim.numpts
            || !std::isfinite (sim.downgrade_factor))
        {
            COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET
        ...
        if (sim.downgrade_factor > 1
            && (sim.numpts % parallel.grid_size ()[0]
                || sim.numpts % parallel.grid_size ()[1]
                || (sim.numpts / parallel.grid_size ()[0])
                       % sim.downgrade_factor
                || (sim.numpts / parallel.grid_size ()[1])
                       % sim.downgrade_factor))
    ...
    parseParameter (params, numparam, "snapshot redshifts", sim.z_snapshot,
                    sim.num_snapshot);
    if (sim.num_snapshot > 0)
        qsort ((void *)sim.z_snapshot, (size_t)sim.num_snapshot,
               sizeof (double), sort_descending);

    parseParameter (params, numparam, "Pk redshifts", sim.z_pk, sim.num_pk);
    if (sim.num_pk > 0)
        qsort ((void *)sim.z_pk, (size_t)sim.num_pk, sizeof (double),
               sort_descending);

    parseParameter (params, numparam, "hibernation redshifts", sim.z_restart,
                    sim.num_restart);
    if (sim.num_restart > 0)
        qsort ((void *)sim.z_restart, (size_t)sim.num_restart, sizeof (double),
               sort_descending);

    parseParameter (params, numparam, "hibernation wallclock limit",
                    sim.wallclocklimit);

    parseFieldSpecifiers (params, numparam, "lightcone outputs",
                          sim.out_lightcone[0]);
    parseFieldSpecifiers (params, numparam, "snapshot outputs",
                          sim.out_snapshot);
    parseFieldSpecifiers (params, numparam, "Pk outputs", sim.out_pk);

    if (!parseParameter (params, numparam, "lightcone pixel factor",
                         sim.pixelfactor[0]))
        sim.pixelfactor[0] = 0.5;

    if (!parseParameter (params, numparam, "lightcone shell factor",
                         sim.shellfactor[0]))
        sim.shellfactor[0] = 1.;

    if (!parseParameter (params, numparam, "lightcone covering",
                         sim.covering[0]))
    ...
    if (parseParameter (params, numparam, "lightcone Nside", sim.Nside[0], i))
    {
        ...
        {
            COUT << COLORTEXT_YELLOW << " /!\\ warning" << COLORTEXT_RESET
                 << ": lightcone Nside parameter out of bounds, "
                    "assuming "
                    "minimum "
                    "value Nside=2"
                 << std::endl;
        ...
        else
        {
            ...
            {
                COUT << COLORTEXT_YELLOW << " /!\\ warning" << COLORTEXT_RESET
                     << ": lightcone Nside parameter was found to "
                        "be no power "
                        "of "
    ...
    {
        sim.Nside[0][0] = 2;
        for (sim.Nside[0][1] = 2; sim.Nside[0][1] < sim.numpts;
             sim.Nside[0][1] *= 2)
            ;
    }
    ...
    i = 3;
    if (parseParameter (params, numparam, "lightcone vertex",
                        sim.lightcone[0].vertex, i))
    {
        if (i != 3 || sim.lightcone[0].vertex[0] < 0.
            || sim.lightcone[0].vertex[0] >= sim.boxsize
            || sim.lightcone[0].vertex[1] < 0.
            || sim.lightcone[0].vertex[1] >= sim.boxsize
            || sim.lightcone[0].vertex[2] < 0.
            || sim.lightcone[0].vertex[2] >= sim.boxsize)
        {
            COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET
        ...
        else
        {
            ...
            if (parseParameter (params, numparam,
                                "lightcone opening half-angle",
                                sim.lightcone[0].opening))
            {
                if (sim.lightcone[0].opening > 180.
                    || sim.lightcone[0].opening <= 0.)
                {
                    ...
                    sim.lightcone[0].opening
                        = cos (sim.lightcone[0].opening * M_PI / 180.);
            }
            else
                sim.lightcone[0].opening = -1.;

            i = 2;
            if (parseParameter (params, numparam, "lightcone distance",
                                sim.lightcone[0].distance, i))
            {
                sim.lightcone[0].distance[0] /= sim.boxsize;
                if (i > 1)
                {
                    sim.lightcone[0].distance[1] /= sim.boxsize;
                    qsort ((void *)sim.lightcone[0].distance, 2,
                           sizeof (double), sort_descending);
                }
                else
            ...
            if (!parseParameter (params, numparam, "lightcone redshift",
                                 sim.lightcone[0].z))
                sim.lightcone[0].z = 0.;

            i = 3;
            if (parseParameter (params, numparam, "lightcone direction",
                                sim.lightcone[0].direction, i))
            {
                if (i == 2)
                {
                    tmp = sin (sim.lightcone[0].direction[0] * M_PI / 180.);
                    sim.lightcone[0].direction[2]
                        = cos (sim.lightcone[0].direction[0] * M_PI / 180.);
                    sim.lightcone[0].direction[0]
                        = tmp
                          * cos (sim.lightcone[0].direction[1] * M_PI / 180.);
                    sim.lightcone[0].direction[1]
                        = tmp
                          * sin (sim.lightcone[0].direction[1] * M_PI / 180.);
                }
                else if (i == 3)
                {
                    tmp = sqrt (sim.lightcone[0].direction[0]
                                    * sim.lightcone[0].direction[0]
                                + sim.lightcone[0].direction[1]
                                      * sim.lightcone[0].direction[1]
                                + sim.lightcone[0].direction[2]
    ...
    {
        for (sim.num_lightcone = 0; sim.num_lightcone < MAX_OUTPUTS;
             sim.num_lightcone++)
        {
            sprintf (par_string, "lightcone %d vertex", sim.num_lightcone);
            i = 3;
            if (parseParameter (params, numparam, par_string,
                                sim.lightcone[sim.num_lightcone].vertex, i))
            {
                if (i != 3 || sim.lightcone[sim.num_lightcone].vertex[0] < 0.
                    || sim.lightcone[sim.num_lightcone].vertex[0] >= sim.boxsize
                    || sim.lightcone[sim.num_lightcone].vertex[1] < 0.
                    || sim.lightcone[sim.num_lightcone].vertex[1] >= sim.boxsize
                    || sim.lightcone[sim.num_lightcone].vertex[2] < 0.
                    || sim.lightcone[sim.num_lightcone].vertex[2]
                           >= sim.boxsize)
                {
                    COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET
                         << ": parsing of lightcone geometry "
                ...
                else
                {
                    ...
                    sprintf (par_string, "lightcone %d outputs",
                             sim.num_lightcone);
                    parseFieldSpecifiers (params, numparam, par_string,
                                          sim.out_lightcone[sim.num_lightcone]);

                    sprintf (par_string, "lightcone %d opening half-angle",
                             sim.num_lightcone);
                    if (parseParameter (
                            params, numparam, par_string,
                            sim.lightcone[sim.num_lightcone].opening))
                    {
                        if (sim.lightcone[sim.num_lightcone].opening > 180.
                            || sim.lightcone[sim.num_lightcone].opening <= 0.)
                        {
                            COUT << COLORTEXT_YELLOW << " /!\\ warning"
                                 << COLORTEXT_RESET
                                 << ": lightcone "
                            ...
                            sim.lightcone[sim.num_lightcone].opening
                                = cos (sim.lightcone[sim.num_lightcone].opening
                                       * M_PI / 180.);
                    }
                    else
                        sim.lightcone[sim.num_lightcone].opening = -1.;

                    sprintf (par_string, "lightcone %d distance",
                             sim.num_lightcone);
                    i = 2;
                    if (parseParameter (
                            params, numparam, par_string,
                            sim.lightcone[sim.num_lightcone].distance, i))
                    {
                        sim.lightcone[sim.num_lightcone].distance[0]
                            /= sim.boxsize;
                        if (i > 1)
                        {
                            sim.lightcone[sim.num_lightcone].distance[1]
                                /= sim.boxsize;
                            qsort ((void *)sim.lightcone[sim.num_lightcone]
                                       .distance,
                                   2, sizeof (double), sort_descending);
                        }
                        else
                    ...
                    sprintf (par_string, "lightcone %d redshift",
                             sim.num_lightcone);
                    if (!parseParameter (params, numparam, par_string,
                                         sim.lightcone[sim.num_lightcone].z))
                        sim.lightcone[sim.num_lightcone].z = 0.;

                    sprintf (par_string, "lightcone %d direction",
                             sim.num_lightcone);
                    i = 3;
                    if (parseParameter (
                            params, numparam, par_string,
                            sim.lightcone[sim.num_lightcone].direction, i))
                    {
                        if (i == 2)
                        {
                            tmp = sin (
                                sim.lightcone[sim.num_lightcone].direction[0]
                                * M_PI / 180.);
                            sim.lightcone[sim.num_lightcone].direction[2]
                                = cos (sim.lightcone[sim.num_lightcone]
                                           .direction[0]
                                       * M_PI / 180.);
                            sim.lightcone[sim.num_lightcone].direction[0]
                                = tmp
                                  * cos (sim.lightcone[sim.num_lightcone]
                                             .direction[1]
                                         * M_PI / 180.);
                            sim.lightcone[sim.num_lightcone].direction[1]
                                = tmp
                                  * sin (sim.lightcone[sim.num_lightcone]
                                             .direction[1]
                                         * M_PI / 180.);
                        }
                        else if (i == 3)
                        {
                            tmp = sqrt (
                                sim.lightcone[sim.num_lightcone].direction[0]
                                    * sim.lightcone[sim.num_lightcone]
                                          .direction[0]
                                + sim.lightcone[sim.num_lightcone].direction[1]
                                      * sim.lightcone[sim.num_lightcone]
                                            .direction[1]
                    ...
                    sprintf (par_string, "lightcone %d pixel factor",
                             sim.num_lightcone);
                    parseParameter (params, numparam, par_string,
                                    sim.pixelfactor[sim.num_lightcone]);
                    sprintf (par_string, "lightcone %d shell factor",
                             sim.num_lightcone);
                    parseParameter (params, numparam, par_string,
                                    sim.shellfactor[sim.num_lightcone]);
                    sprintf (par_string, "lightcone %d covering",
                             sim.num_lightcone);
                    parseParameter (params, numparam, par_string,
                                    sim.covering[sim.num_lightcone]);
                    sprintf (par_string, "lightcone %d Nside",
                             sim.num_lightcone);
                    i = 2;
                    if (parseParameter (params, numparam, par_string,
                                        sim.Nside[sim.num_lightcone], i))
                    {
                        if (i < 1)
                        {
                            COUT << COLORTEXT_YELLOW << " /!\\ warning"
                        ...
                        {
                            for (i = 2; i < sim.Nside[sim.num_lightcone][0];
                                 i *= 2)
                                ;
                        ...
                        if (sim.Nside[sim.num_lightcone][1]
                            < sim.Nside[sim.num_lightcone][0])
                        {
                            COUT << COLORTEXT_YELLOW << " /!\\ warning"
                                 << COLORTEXT_RESET
                        ...
                        {
                            for (i = 2; i < sim.Nside[sim.num_lightcone][1];
                                 i *= 2)
                                ;
    ...
    if ((sim.num_snapshot <= 0 || sim.out_snapshot == 0)
    ...
    if (!parseParameter (params, numparam, "m_ncdm", cosmo.m_ncdm,
                         cosmo.num_ncdm))
    {
    ...
    {
        cosmo.Omega_ncdm[i] = cosmo.m_ncdm[i] * cosmo.deg_ncdm[i]
                              / P_NCDM_MASS_OMEGA / cosmo.h / cosmo.h;
    }
    ...
    {
        cosmo.Omega_ur
            *= (7. / 8.) * std::pow (4. / 11., 4. / 3.) * cosmo.Omega_g;
    }
    ...
    {
        cosmo.Omega_ur
            = P_N_UR * (7. / 8.) * std::pow (4. / 11., 4. / 3.) * cosmo.Omega_g;
    }
    ...
    if (!parseParameter (params, numparam, "switch delta_rad",
                         sim.z_switch_deltarad))
        sim.z_switch_deltarad = 0.;

    i = MAX_PCL_SPECIES - 2;
    if (!parseParameter (params, numparam, "switch delta_ncdm",
                         sim.z_switch_deltancdm, i))
    {
        for (i = 0; i < MAX_PCL_SPECIES - 2; i++)
            sim.z_switch_deltancdm[i]
                = (ic.numtile[(sim.baryon_flag == 1) ? 2 + i : 1 + i] > 0)
                      ? sim.z_in
                      : 0.;
    }
    else
    ...
    i = MAX_PCL_SPECIES - 2;
    if (!parseParameter (params, numparam, "switch B ncdm", sim.z_switch_Bncdm,
                         i))
    {
        for (i = 0; i < MAX_PCL_SPECIES - 2; i++)
