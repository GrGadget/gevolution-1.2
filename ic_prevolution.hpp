    ...
    PlanFFT<Cplx> *plan_Sij, parameter *params, int &numparam)
{
    ...
    COUT << " initial particle horizon tau = " << tau * sim.numpts
         << " lattice units." << endl;

    initializeCLASSstructures (sim, ic, cosmo, class_background, class_thermo,
                               class_perturbs, params, numparam);

    loadTransferFunctions (class_background, class_perturbs, tk_d1, tk_d2, NULL,
                           sim.boxsize, ic.z_ic, cosmo.h);
    ...
    for (i = 0; i < tk_d1->size; i++)
    {
        temp1[i]
            = -tk_d1->y[i] * M_PI
              * sqrt (Pk_primordial (tk_d1->x[i] * cosmo.h / sim.boxsize, ic)
                      / tk_d1->x[i])
              / tk_d1->x[i];
        temp2[i]
            = (tk_d2->y[i] - tk_d1->y[i]) * M_PI
              * sqrt (Pk_primordial (tk_d1->x[i] * cosmo.h / sim.boxsize, ic)
                      / tk_d1->x[i])
              / tk_d1->x[i];
    }
    ...
    for (p = 0; p < cosmo.num_ncdm; p++)
    {
        ...
        loadHomogeneousTemplate (
            ic.pclfile[((sim.baryon_flag == 1) ? 2 : 1) + p],
        ...
        loadTransferFunctions (class_background, class_perturbs, tk_d1, tk_t1,
                               ncdm_name, sim.boxsize, ic.z_ic, cosmo.h);

        if (tk_d1 == NULL || tk_t1 == NULL)
        {
            COUT << " error: ncdm transfer function was empty! (species " << p
                 << ")" << endl;
            parallel.abortForce ();
        }

        rescale = pow (cosmo.Omega_g * cosmo.h * cosmo.h / C_PLANCK_LAW, 0.25)
        ...
        for (i = 0; i < tk_t1->size; i++)
            temp2[i] = -4.2018325 * rescale * tk_t1->y[i] * M_PI
                       * sqrt (Pk_primordial (
                                   tk_t1->x[i] * cosmo.h / sim.boxsize, ic)
                               / tk_t1->x[i])
                       / tk_t1->x[i];
        ...
        pcls_ncdm_info[p].mass
            = cosmo.Omega_ncdm[p]
              / (Real) (
                    sim.numpcl[((sim.baryon_flag == 1) ? 2 : 1) + p]
                    * (long)ic.numtile[((sim.baryon_flag == 1) ? 2 : 1) + p]
                    * (long)ic.numtile[((sim.baryon_flag == 1) ? 2 : 1) + p]
                    * (long)ic.numtile[((sim.baryon_flag == 1) ? 2 : 1) + p]);
        ...
        COUT << " " << sim.numpcl[((sim.baryon_flag == 1) ? 2 : 1) + p]
        ...
        plan_source->execute (FFT_FORWARD);
        generateDisplacementField (*scalarFT, 0., tk_d2, (unsigned int)ic.seed,
                                   ic.flags & ICFLAG_KSPHERE);
        plan_chi->execute (FFT_BACKWARD);
        chi->updateHalo ();
        gsl_spline_free (tk_d2);
        ...
        mean_q = applyMomentumDistribution (
            pcls_ncdm + p, (unsigned int)(ic.seed + p), rescale, chi);
        parallel.sum (mean_q);
        COUT << " species " << p + 1
             << " Fermi-Dirac distribution had mean q/m = "
             << mean_q / sim.numpcl[((sim.baryon_flag == 1) ? 2 : 1) + p]
             << endl;
    }
    ...
    do
    {
        if (cycle > 0 && (1. / a < ic.z_relax + 1. || 1. / a <= sim.z_in + 1.))
        {
            if (relax_cycles == 0) // initialize cold species
            {
                ...
                if (ic.flags & ICFLAG_CORRECT_DISPLACEMENT)
                    generateCICKernel (*source, sim.numpcl[0], pcldata,
                                       ic.numtile[0]);
                else
                    generateCICKernel (*source);
                ...
                loadTransferFunctions (class_background, class_perturbs, tk_d1,
                                       tk_t1, "cdm", sim.boxsize, (1. / a) - 1.,
                                       cosmo.h);

                if (sim.baryon_flag > 0)
                {
                    loadTransferFunctions (class_background, class_perturbs,
                                           tk_d2, tk_t2, "b", sim.boxsize,
                                           (1. / a) - 1., cosmo.h);

                    if (tk_d2->size != tk_d1->size)
                    {
                        COUT << " error: baryon transfer "
                ...
                {
                    if (8. * cosmo.Omega_b / (cosmo.Omega_cdm + cosmo.Omega_b)
                        > 1.)
                    {
                        for (i = 0; i < tk_d1->size; i++)
                        {
                        ...
                        gsl_spline_free (tk_d1);
                        gsl_spline_free (tk_t1);
                        tk_d1 = gsl_spline_alloc (gsl_interp_cspline,
                                                  tk_d2->size);
                        tk_t1 = gsl_spline_alloc (gsl_interp_cspline,
                                                  tk_d2->size);
                        gsl_spline_init (tk_d1, tk_d2->x, temp1, tk_d2->size);
                        gsl_spline_init (tk_t1, tk_d2->x, temp2, tk_d2->size);
                    }
                    else
                    {
                        ...
                        gsl_spline_free (tk_d2);
                        gsl_spline_free (tk_t2);
                        tk_d2 = gsl_spline_alloc (gsl_interp_cspline,
                                                  tk_d1->size);
                        tk_t2 = gsl_spline_alloc (gsl_interp_cspline,
                                                  tk_d1->size);
                               ...
                               <= 1.))
                {
                    ...
                    tk_t1 = gsl_spline_alloc (gsl_interp_cspline,
                                              phispline2->size);
                    gsl_spline_init (tk_d1, phispline2->x, temp1,
                                     phispline2->size);
                    gsl_spline_init (tk_t1, phispline2->x, temp2,
                                     phispline2->size);
                }
                ...
                pcls_cdm->initialize (pcls_cdm_info, pcls_cdm_dataType,
                                      &(phi->lattice ()), boxSize);
                ...
                if (sim.baryon_flag == 3)
                    ...
                    pcls_cdm->moveParticles (displace_pcls_ic_basic, 1., &phi,
                                             1, NULL, &max_displacement, &i, 1);

                sim.numpcl[0] *= (long)ic.numtile[0] * (long)ic.numtile[0]
                                 * (long)ic.numtile[0];

                COUT << " " << sim.numpcl[0]
                     << " cdm particles initialized: maximum "
                        "displacement = "
                     << max_displacement * sim.numpts << " lattice units."
                     << endl;
                ...
                if (sim.baryon_flag == 1)
                {
                    loadHomogeneousTemplate (ic.pclfile[1], sim.numpcl[1],
                                             pcldata);

                    if (pcldata == NULL)
                    {
                        COUT << " error: particle data was "
                                "empty!"
                             << endl;
                        parallel.abortForce ();
                    }
                    ...
                    if (ic.flags & ICFLAG_CORRECT_DISPLACEMENT)
                    {
                        generateCICKernel (*chi, sim.numpcl[1], pcldata,
                                           ic.numtile[1]);
                    ...
                    sim.numpcl[1] *= (long)ic.numtile[1] * (long)ic.numtile[1]
                                     * (long)ic.numtile[1];

                    COUT << " " << sim.numpcl[1]
                         << " baryon particles initialized: "
                            "maximum "
                            "displacement "
                            "= "
                         << max_displacement * sim.numpts << " lattice units."
                ...
                if (sim.baryon_flag == 3)
                    maxvel[0] = pcls_cdm->updateVel (initialize_q_ic_basic, 1.,
                                                     ic_fields, 2)
                                / a;
                else
                    maxvel[0] = pcls_cdm->updateVel (initialize_q_ic_basic, 1.,
                ...
                if (sim.baryon_flag == 1)
                    maxvel[1]
                        = pcls_b->updateVel (initialize_q_ic_basic, 1., &chi, 1)
                          / a;

                if (sim.baryon_flag > 1)
                    sim.baryon_flag = 0;

                generateRealization (*scalarFT, 0., phispline,
                                     (unsigned int)ic.seed,
                                     ic.flags & ICFLAG_KSPHERE, 0);
                plan_phi->execute (FFT_BACKWARD);
                phi->updateHalo ();
                ...
                if (1. / a <= sim.z_in + 1.)
                {
                    generateRealization (*scalarFT, 0., chispline,
                                         (unsigned int)ic.seed,
                                         ic.flags & ICFLAG_KSPHERE, 0);
                    plan_chi->execute (FFT_BACKWARD);
                    chi->updateHalo ();
                    ...
                    for (p = 0; p < cosmo.num_ncdm; p++)
                    {
                        if (ic.numtile[1 + sim.baryon_flag + p] < 1)
                            ...
                            maxvel[1 + sim.baryon_flag + p]
                                = pcls_ncdm[p].updateVel (
                                    update_q, dtau_old / 2., ic_fields, 2, &a);
                    }
            ...
            {
                loadTransferFunctions (class_background, class_perturbs, tk_d1,
                                       tk_t1, "g", sim.boxsize, ic.z_ic,
                                       cosmo.h);

                for (i = 0; i < tk_d1->size; i++)
                    temp1[i]
                        -= tk_d1->y[i] * cosmo.Omega_g * M_PI
                           * sqrt (Pk_primordial (
                                       tk_d1->x[i] * cosmo.h / sim.boxsize, ic)
                                   / tk_d1->x[i])
                           / tk_d1->x[i] / a;
            ...
            {
                loadTransferFunctions (class_background, class_perturbs, tk_d1,
                                       tk_t1, "ur", sim.boxsize, ic.z_ic,
                                       cosmo.h);

                for (i = 0; i < tk_d1->size; i++)
                    temp1[i]
                        -= tk_d1->y[i] * cosmo.Omega_ur * M_PI
                           * sqrt (Pk_primordial (
                                       tk_d1->x[i] * cosmo.h / sim.boxsize, ic)
                                   / tk_d1->x[i])
                           / tk_d1->x[i] / a;
            ...
            for (p = 0; p < cosmo.num_ncdm; p++)
            {
                sprintf (ncdm_name, "ncdm[%d]", p);
                loadTransferFunctions (class_background, class_perturbs, tk_d1,
                                       tk_t1, ncdm_name, sim.boxsize, ic.z_ic,
                                       cosmo.h);
            ...
            if (cosmo.num_ncdm > 0 || cosmo.Omega_rad > 0)
            {
                ...
                generateRealization (*scalarFT, 0., tk_d1,
                                     (unsigned int)ic.seed,
                                     ic.flags & ICFLAG_KSPHERE);
                gsl_spline_free (tk_d1);
            }
            ...
            prepareFTsource<Real> (
                *phi, *chi, *source, cosmo.Omega_cdm + cosmo.Omega_b, *source,
                3. * Hconf (a, fourpiG, cosmo) * dx * dx / dtau_old,
                fourpiG * dx * dx / a,
                3. * Hconf (a, fourpiG, cosmo) * Hconf (a, fourpiG, cosmo) * dx
                    * dx);
            plan_source->execute (FFT_FORWARD);
            solveModifiedPoissonFT (*scalarFT, *scalarFT, 1. / (dx * dx),
                                    3. * Hconf (a, fourpiG, cosmo) / dtau_old);
            plan_chi->execute (FFT_BACKWARD); // nonlinear phi temporarily
                                              // stored in chi

            relax_cycles++;
        }

        loadTransferFunctions (class_background, class_perturbs, tk_d1, tk_d2,
                               NULL, sim.boxsize, (1. / a) - 1., cosmo.h);

        for (i = 0; i < tk_d1->size; i++)
        {
            temp1[i] = -tk_d1->y[i] * M_PI
                       * sqrt (Pk_primordial (
                                   tk_d1->x[i] * cosmo.h / sim.boxsize, ic)
                               / tk_d1->x[i])
                       / tk_d1->x[i];
            temp2[i] = (tk_d2->y[i] - tk_d1->y[i]) * M_PI
                       * sqrt (Pk_primordial (
                                   tk_d1->x[i] * cosmo.h / sim.boxsize, ic)
                               / tk_d1->x[i])
                       / tk_d1->x[i];
        ...
        generateRealization (*scalarFT, 0., phispline, (unsigned int)ic.seed,
                             ic.flags & ICFLAG_KSPHERE, 0);
        plan_phi->execute (FFT_BACKWARD);
        ...
        if (relax_cycles == 1)
        {
            ...
            sprintf (filename, "%s%sICtarget_phi.dat", sim.output_path,
                     sim.basename_pk);
            writePowerSpectrum (
                kbin, power, kscatter, pscatter, occupation, sim.numbins,
                sim.boxsize, (Real)numpts3d * (Real)numpts3d * 2. * M_PI * M_PI,
                filename, "power spectrum of phi", a);
            plan_chi->execute (FFT_FORWARD);
            extractPowerSpectrum (*scalarFT, kbin, power, kscatter, pscatter,
                                  occupation, sim.numbins, false, KTYPE_LINEAR);
            sprintf (filename, "%s%sICactual_phi.dat", sim.output_path,
                     sim.basename_pk);
            writePowerSpectrum (
                kbin, power, kscatter, pscatter, occupation, sim.numbins,
                sim.boxsize, (Real)numpts3d * (Real)numpts3d * 2. * M_PI * M_PI,
        ...
        if (relax_cycles > 0)
        {
            for (x.first (); x.test (); x.next ()) // interpolate between linear
                                                   // and nonlinear solution
                (*phi) (x)
                    = (((1. / (sim.z_in + 1.)) - a) * (*phi) (x)
                       + (a - (1. / (ic.z_relax + 1.))) * (*chi) (x))
                      / ((1. / (sim.z_in + 1.)) - (1. / (ic.z_relax + 1.)));
        }
        phi->updateHalo ();
        ...
        if (cycle == 0)
        {
            ...
            extractPowerSpectrum (*scalarFT, kbin, power, kscatter, pscatter,
                                  occupation, sim.numbins, false, KTYPE_LINEAR);
            sprintf (filename, "%s%sICinit_phi.dat", sim.output_path,
                     sim.basename_pk);
            writePowerSpectrum (
                kbin, power, kscatter, pscatter, occupation, sim.numbins,
                sim.boxsize, (Real)numpts3d * (Real)numpts3d * 2. * M_PI * M_PI,
        ...
        generateRealization (*scalarFT, 0., chispline, (unsigned int)ic.seed,
                             ic.flags & ICFLAG_KSPHERE, 0);
        plan_chi->execute (FFT_BACKWARD);
        ...
        if (relax_cycles > 0)
        {
            ...
            plan_source->execute (FFT_BACKWARD); // nonlinear chi temporarily
                                                 // stored in source

            for (x.first (); x.test (); x.next ()) // interpolate between linear
                                                   // and nonlinear solution
                (*chi) (x)
                    += (a - (1. / (ic.z_relax + 1.))) * (*source) (x)
                       / ((1. / (sim.z_in + 1.)) - (1. / (ic.z_relax + 1.)));
        }
        chi->updateHalo ();
        ...
        if (relax_cycles > 0)
        {
            maxvel[0] = pcls_cdm->updateVel (update_q, (dtau + dtau_old) / 2.,
                                             ic_fields, 1, &a);
            if (sim.baryon_flag)
                maxvel[1] = pcls_b->updateVel (update_q, (dtau + dtau_old) / 2.,
                                               ic_fields, 1, &a);
        }
        ...
        for (p = 0; p < cosmo.num_ncdm; p++)
        {
            if (ic.numtile[((sim.baryon_flag == 1) ? 2 : 1) + p] < 1)
                ...
                maxvel[((sim.baryon_flag == 1) ? 2 : 1) + p]
                    = pcls_ncdm[p].updateVel (update_q, (dtau + dtau_old) / 2.,
                                              ic_fields, 2, &a);
        }
        ...
        if (relax_cycles > 0 && ic.Cf > sim.Cf)
            dtau = (((1. / (sim.z_in + 1.)) - a) * ic.Cf * dx
                    + (a - (1. / (ic.z_relax + 1.)))
                          * ((sim.Cf * dx
                              < sim.steplimit / Hconf (a, fourpiG, cosmo))
                                 ? (sim.Cf * dx)
                                 : (sim.steplimit / Hconf (a, fourpiG, cosmo))))
                   / ((1. / (sim.z_in + 1.)) - (1. / (ic.z_relax + 1.)));
        else if (relax_cycles > 0
                 && ic.Cf * dx > sim.steplimit / Hconf (a, fourpiG, cosmo))
            dtau = (((1. / (sim.z_in + 1.)) - a) * ic.Cf * dx
                    + (a - (1. / (ic.z_relax + 1.))) * sim.steplimit
    ...
    if (sim.gr_flag == 0)
    {
        ...
        loadTransferFunctions (class_background, class_perturbs, tk_d2, tk_t2,
                               NULL, sim.boxsize, (1. / a) - 1., cosmo.h);

        for (i = 0; i < tk_d2->size; i++)
            temp1[i] = -tk_d2->y[i] * M_PI
                       * sqrt (Pk_primordial (
        ...
        i = MAX;
        pcls_cdm->moveParticles (displace_pcls_ic_basic, 1., &source, 1, NULL,
                                 &max_displacement, &i, 1);
        COUT << " Poisson gauge -> N-body gauge, cdm particles maximum "
                "displacement = "
        ...
        if (sim.baryon_flag)
        {
            ...
            pcls_b->moveParticles (displace_pcls_ic_basic, 1., &source, 1, NULL,
                                   &max_displacement, &i, 1);
            COUT << " Poisson gauge -> N-body gauge, baryon particles "
                    "maximum "
                    "displacement = "
                 << max_displacement * sim.numpts << " lattice units." << endl;
        }
        ...
        for (p = 0; p < cosmo.num_ncdm; p++)
        {
            if (ic.numtile[((sim.baryon_flag == 1) ? 2 : 1) + p] < 1)
                continue;
            pcls_ncdm[p].moveParticles (displace_pcls_ic_basic, 1., &source, 1,
                                        NULL, &max_displacement, &i, 1);
            COUT << " Poisson gauge -> N-body gauge, ncdm species " << p + 1
                 << " maximum displacement = " << max_displacement * sim.numpts
                 << " lattice units." << endl;
        }

        loadTransferFunctions (class_background, class_perturbs, tk_d2, tk_t2,
                               NULL, sim.boxsize, (1. / (0.99 * a)) - 1.,
                               cosmo.h);
        ...
        for (p = 0; p < cosmo.num_ncdm; p++)
        {
            ...
            COUT << " Poisson gauge -> N-body gauge, ncdm species " << p + 1
                 << " maximum velocity: " << maxvel[p + 1 + sim.baryon_flag]
                 << " -> ";
            maxvel[p + 1 + sim.baryon_flag]
                = pcls_ncdm[p].updateVel (update_q_Newton, 1., &source, 1, &a);
    ...
    {
        COUT << " error: calling perturb_free from CLASS library failed!"
             << endl
             << " following error message was passed: "
             << class_perturbs.error_message << endl;
        parallel.abortForce ();
    }

    if (thermodynamics_free (&class_thermo) == _FAILURE_)
    {
        COUT << " error: calling thermodynamics_free from CLASS library "
                "failed!"
             << endl
             << " following error message was passed: "
             << class_thermo.error_message << endl;
        parallel.abortForce ();
    }

    if (background_free (&class_background) == _FAILURE_)
    {
        COUT << " error: calling background_free from CLASS library failed!"
             << endl
             << " following error message was passed: "
             << class_background.error_message << endl;
        parallel.abortForce ();
    }
