...
void projection_T00_project (background &class_background,
                             ...
                             const double fourpiG, double a, double coeff)
{
    gsl_spline *tk1 = NULL;
    gsl_spline *tk2 = NULL;
    double *delta = NULL;
    ...
    {
        loadTransferFunctions (class_background, class_perturbs, tk1, tk2, "g",
                               sim.boxsize, (1. / a) - 1., cosmo.h);
        ...
        {
            delta[i]
                = -tk1->y[i] * coeff * cosmo.Omega_g * M_PI
                  * sqrt (Pk_primordial (tk1->x[i] * cosmo.h / sim.boxsize, ic)
                          / tk1->x[i])
                  / tk1->x[i] / a;
            k[i] = tk1->x[i];
        }
    ...
    if (a < 1. / (sim.z_switch_deltarad + 1.) && cosmo.Omega_ur > 0
        && sim.radiation_flag == 1)
    {
        loadTransferFunctions (class_background, class_perturbs, tk1, tk2, "ur",
                               sim.boxsize, (1. / a) - 1., cosmo.h);
        ...
        if (delta == NULL)
        {
            ...
            for (i = 0; i < n; i++)
            {
                delta[i] = -tk1->y[i] * coeff * cosmo.Omega_ur * M_PI
                           * sqrt (Pk_primordial (
                                       tk1->x[i] * cosmo.h / sim.boxsize, ic)
                                   / tk1->x[i])
                           / tk1->x[i] / a;
                k[i] = tk1->x[i];
            }
    ...
    if (a < 1. && cosmo.Omega_fld > 0 && sim.fluid_flag == 1)
    {
        ...
        if (delta == NULL)
        {
            ...
            for (i = 0; i < n; i++)
            {
                delta[i] = -tk1->y[i] * coeff * Omega_fld * M_PI
                           * sqrt (Pk_primordial (
                                       tk1->x[i] * cosmo.h / sim.boxsize, ic)
                                   / tk1->x[i])
                           / tk1->x[i];
    ...
    {
        if (a < 1. / (sim.z_switch_deltancdm[p] + 1.)
            && cosmo.Omega_ncdm[p] > 0)
        {
            sprintf (ncdm_name, "ncdm[%d]", p);
            loadTransferFunctions (class_background, class_perturbs, tk1, tk2,
                                   ncdm_name, sim.boxsize, (1. / a) - 1.,
                                   cosmo.h);
    ...
    if (n > 0)
    {
        if (sim.gr_flag == gravity_theory::Newtonian) // add gauge correction for N-body gauge
        {
            ...
            for (i = 0; i < n; i++)
                delta[i] -= coeff
                            * (4. * Omega_rad / a + 3. * Omega_ncdm
                               + 3. * (1. + cosmo.w0_fld) * Omega_fld)
                            * rescale * M_PI * tk2->y[i]
                            * sqrt (Pk_primordial (
                                        tk2->x[i] * cosmo.h / sim.boxsize, ic)
                                    / tk2->x[i])
                            / tk2->x[i] / tk2->x[i] / tk2->x[i];
        ...
        generateRealization (scalarFT, 0., tk1, (unsigned int)ic.seed,
                         ...
                         double coeff)
{
    ...
    loadTransferFunctions (class_background, class_perturbs, tk1, tk2, NULL,
                           sim.boxsize, (1. / a) - 1., cosmo.h);

    chi = (double *)malloc (tk1->size * sizeof (double));

    for (i = 0; i < tk1->size; i++)
        chi[i] = (tk2->y[i] - tk1->y[i]) * coeff * M_PI
                 * sqrt (Pk_primordial (tk1->x[i] * cosmo.h / sim.boxsize, ic)
                         / tk1->x[i])
                 / tk1->x[i];
    ...
    if (sim.gr_flag == gravity_theory::Newtonian) // add gauge correction for N-body gauge
    {
        ...
        loadTransferFunctions (class_background, class_perturbs, tk1, tk2, NULL,
                               sim.boxsize, (1. / (1.005 * a)) - 1., cosmo.h);
        ...
        loadTransferFunctions (class_background, class_perturbs, tk1, tk2, NULL,
                               sim.boxsize, (1. / (1.01 * a)) - 1., cosmo.h);
        ...
        loadTransferFunctions (class_background, class_perturbs, tk1, tk2, NULL,
                               sim.boxsize, (1. / (0.995 * a)) - 1., cosmo.h);
        ...
        loadTransferFunctions (class_background, class_perturbs, tk1, tk2, NULL,
                               sim.boxsize, (1. / (0.99 * a)) - 1., cosmo.h);
        ...
        Hconf1 = (8.0802 * Hconf4 - 7.9202 * Hconf2 - 1.0201 * Hconf5
                  + 0.9801 * Hconf1)
                 / 12.;
        ...
        for (i = 0; i < tk1->size; i++)
            chi[i]
                += 10000. * Hconf3
                   * (Hconf1 * (8. * l4[i] - 8. * l2[i] - l5[i] + l1[i])
                      + Hconf3
                            * (16. * l4[i] + 16. * l2[i] - 30. * l3[i] - l5[i]
                               - l1[i]))
                   * M_PI
                   * sqrt (Pk_primordial (tk1->x[i] * cosmo.h / sim.boxsize, ic)
                           / tk1->x[i])
                   / tk1->x[i] / tk1->x[i] / tk1->x[i];
    ...
    generateRealization (scalarFT, 0., tk2, (unsigned int)ic.seed,
